...
7. **Rebuild `hosts/` and pass the drift gate.** Run `uv run scripts/build_hosts.py`, commit the rebuilt trees, and confirm `uv run scripts/build_hosts.py --check` passes; the same check runs as the `drift-gate` status on your pull request.
