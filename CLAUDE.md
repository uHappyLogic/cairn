...
agents/submit-task.md — task-authoring subagent invoked by populate-backlog (bulk)
...
populate-backlog                ← decomposes requirements.md into ordered briefs, proves coverage, spawns submit-task agent per brief
...
submit-task                     ← (agent, in agents/) bulk per-task authoring for populate-backlog: runs shared/submit-procedure.md in isolation, returns DONE/FAILED
...
- The task body template (`##` title, description, optional **Provides**, optional **Notes**, **Success**, trailing `---`) lives in exactly one place: `shared/submit-procedure.md`. The `submit-task` skill, the `submit-task` agent, and `populate-backlog` must not duplicate it. There is deliberately **no** `Steps` section — see the altitude invariant below.
