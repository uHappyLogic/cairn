...
agents/try-answer-question-by-principle.md — read-only candidate-elimination subagent invoked by the try-answer-questions-by-principle orchestrator; returns a verdict only, never edits documents
...
shared/answer-procedure.md — single source of truth for recording an answer (locate question block by Short Title → fold decision into ## Implementation decisions → cascade), referenced by answer-open-question and the try-answer-questions-by-principle orchestrator
...
try-answer-questions-by-principle ← orchestrator: sweeps open/deferred questions, auto-answers only those a confirmed principle resolves, one commit per answer; run after highlight
...
- Two orchestrators commit; individual skills never commit — including the `implement-backlog-task` skill, which leaves an ad-hoc task's changes staged for the user. `implement-backlog-tasks` commits after each successful task. `try-answer-questions-by-principle` commits exactly **one auto-answer per commit** — subject `Principle-based-answer: <Short Title>` (the answered question's handle), with one repeated `Answer-Principle: <Short Title>` trailer line per load-bearing principle.
