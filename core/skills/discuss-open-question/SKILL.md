...
- **The block is bare.** Read and follow `{{PLUGIN_ROOT}}/shared/alternatives-procedure.md`, passing the located block as its **QUESTION** input, and enumerate the options here — the 2–4 honest alternatives, each with what-it-is / key advantage / key drawback. That enumerated set is the option set for the rest of the deliberation. It is this conversation's and is written nowhere; the alternatives pass supplies the block's frozen set when it next runs.
