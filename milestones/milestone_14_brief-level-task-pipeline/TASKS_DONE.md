...
- `uv run scripts/migrate_skills_to_agy.py`, run from the repository root, exits 0 (`EXIT=0`), printing the expected `No source MCP config found at .mcp.json` line and `Successfully migrated 21 skills`.
- `.agents/plugins/cairn/agents/submit-task.md` does not exist after the run — the generated twin of the retired source agent is gone.
- `.agents/plugins/cairn/agents/` contains exactly one `.md` file per file in the root `agents/` directory with matching names (`answer-open-question-with-recommendation.md`, `complete-task.md`, `recommend-open-question.md`) — a name-by-name `diff` of the two listings is empty.
- `.agents/plugins/cairn/skills/` contains exactly one directory per root `skills/` subdirectory other than `migrate-workspace` (21 directories, name-by-name `diff` empty), each holding a `SKILL.md` and no other file.
- Running the script a second time immediately afterwards leaves `.agents/` byte-identical (checksums of every file before and after the second run are equal, second run also exits 0), and `git status --porcelain .agents/` reports only the intended regeneration change set — the deleted `agents/submit-task.md` twin plus the `derive-tasks`, `submit-task`, and `answer-open-question-with-recommendation` skill files — so the tree is unchanged once that change set is committed.
- The regenerated tree carries no stale source references: `grep -rn "submit-procedure" .agents/` and `grep -rn "submit-task agent" .agents/` both return nothing, and the generated `derive-tasks` and `submit-task` skills reference `shared/task-format.md`.
