...
### answer-open-question redirect after extraction

When the record-recommendation mode and its `record the recommendation` sentinel are removed from `answer-open-question`, the skill does not drop the sentinel silently. It retains a small guard that recognizes the answer text exactly equal to the old sentinel `record the recommendation` (whole-string, trim+lowercase) and, instead of recording it, stops without recording and prints a redirect message pointing to `/answer-open-question-with-recommendation`. This intercepts a migrating user's old `<Title>. record the recommendation` command at the exact moment it would otherwise silently record the sentinel phrase as a committed decision, and its redirect message doubles as discovery; the recommend sweep's step-6 pointer plus the README/CLAUDE.md edits remain the primary discovery path, so this guard is targeted migration scaffolding, not a revived feature. It fits the skill's existing clean-stop-and-point pattern (the no-anchor guard and Short-Title mismatch), at the cost of a small sliver of retained sentinel-awareness in a skill meant to become literal-only.
