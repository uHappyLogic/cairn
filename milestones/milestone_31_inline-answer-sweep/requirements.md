...
A contradiction is a recorded pick that runs against any entry in the live `## Decisions` section at fold time, whether that entry existed before the run or was recorded earlier in the same run. This is exactly the answer core's analyse-step question of whether the answer contradicts or supersedes anything already written, so noticing one adds no new analysis and stays noticing rather than judging; the sweep never checks a pick against another block's standing recommendation it has not yet reached. The sweep keeps the conflicting pairs it noticed across the loop and prints them once, beside its terse status line, as one advisory naming each pair: the answered question's Short Title and the Decisions heading it conflicts with. The advisory names identifiers because git does not record which decision a folded answer conflicts with, and without them the user would have to re-read every commit of the run before a revert-then-re-answer.
...
The sweep runs no separate lift of its own: per question it follows `answer-with-recommendation-procedure.md` from its lift step, so each question is lifted exactly once. A failure of that procedure's lift is the per-question skip, and the line that lift prints is held as the commit body, the same way the single-question `answer-open-question-with-recommendation` skill already keeps the procedure's lift print for its commit. The procedure's no-recommendation guard only stops without changing anything and leaves what follows to the caller; the sweep treats that stop as a silent skip, while the run-stopping boundary stays with the mid-question failure decision, which applies only after a fold.
