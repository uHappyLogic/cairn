...
When more than one confirmed principle bears on the same question, the `recommend-open-question` agent renders one `> **Applied principle:** <Short Title>` line per bearing principle, all stacked above the single `> **Recommendation:**` anchor. This extends the single-principle citation-placement rule by pure repetition — no new label and no in-line list syntax — keeping each citation atomic and independently greppable, holding the anchor as the last line, and mirroring the retired sweep's one-citation-per-principle house style.
