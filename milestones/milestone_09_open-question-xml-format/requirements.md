...
### Boundary-line tag contract

The `<open-question …>` opening tag is written on a single physical line that never wraps, with attributes in id-first order — `<open-question id="..." status="open|deferred">` — using double-quoted values (covered by the uniform `&quot;`/`&apos;` entity-escaping). The CLI extracts `id` and `status` by attribute-name-anchored regex (e.g. `id="([^"]*)"`) rather than by position, so locate/extract/remove is independent of attribute order and stays resilient if an attribute is added or reordered. The canonical id-first order — which honors the goal's illustrated `<open-question id="Short Title" status="open|deferred">` and keeps the id (the locate handle) leading — is therefore an authoring convention the matcher does not enforce, not a positional requirement.
