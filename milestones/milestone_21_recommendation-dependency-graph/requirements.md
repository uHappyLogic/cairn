...
<open-question id="Forward dependency declaration" status="open">
  ...
  <alternative id="Guessed option">
    The agent emits a full `&lt;depends-on question="…" option="…"/&gt;` for the un-annotated sibling, inventing the option value it assumed that sibling will land on.
    <advantage>Keeps one uniform tag shape with an option always present, so the answer-time agree/disagree comparison and the gate&apos;s option-matching test need no special case.</advantage>
    <drawback>The guessed option names an alternative id that does not exist yet and will almost never match the ids the later dispatch actually renders, so the gate cannot validate it and the answer-time comparison mis-fires — recording the sibling&apos;s answer would read as a disagreement and strip the dependent even when the guess was substantively right.</drawback>
  </alternative>
  <alternative id="Option-less tag">
    The agent emits `&lt;depends-on question="…"/&gt;` naming the sibling but asserting no option, marking the coupling without claiming an outcome.
    <advantage>Records the coupling honestly and machine-readably without fabricating an id, so a later answer to that sibling can still find and clear its dependents.</advantage>
    <drawback>Introduces a third cascade branch into a procedure whose whole extension is the agree/disagree test — with nothing assumed there is nothing to compare, so every such tag must resolve by fiat (always strip, or always keep), and the gate loses its option-matching check for exactly the tags whose target is least verifiable.</drawback>
  </alternative>
  <alternative id="Backward-only tags">
    No element at all: a `&lt;depends-on&gt;` is emitted only for a sibling that already carries embedded children in the block the agent read, and a forward coupling is expressed as prose inside the affected `&lt;drawback&gt;` or the rationale.
    <advantage>Every `&lt;depends-on&gt;` then has a verifiable target by construction — an existing sibling with existing `&lt;alternative&gt;` ids — so the acceptance gate, the answer-time agree/disagree comparison, and the transitive strip all keep exactly one uniform case, and the tag&apos;s meaning matches what the goal defines it for (a recommendation built on a sibling recommendation already embedded).</advantage>
    <drawback>A recommendation that genuinely turns on a later sibling leaves no machine-readable trace, so answering that sibling strips nothing and a stale rationale can be lifted later; the only guard is the prose contingency a human has to read.</drawback>
  </alternative>
  <alternative id="Defer and requeue">
    The agent returns a defer signal naming the blocking sibling, and the sweep re-queues the question behind it so the dependency becomes a backward one before it is annotated.
    <advantage>Fixes the root cause rather than the notation — the judgment-based most-significant-first order is corrected by the only party that can see the actual coupling.</advantage>
    <drawback>Requires a second return kind alongside the element region and the `FAILED:` line, an outer re-gather loop over a list the goal walks exactly once, and cycle detection for mutually-dependent questions — a large amount of new machinery for a case no recorded milestone has produced.</drawback>
  </alternative>
  <recommendation option="Backward-only tags">Only a backward tag has a target whose `&lt;alternative&gt;` ids already exist, which is what lets the gate validate it and the cascade&apos;s agree/disagree test stay two-outcome; a guessed option would fabricate an id that reliably mis-compares, and the forward case is unevidenced — the milestone&apos;s own history records recommendation-vs-override coupling, never sweep-vs-sweep — so the residual risk is best left to prose in the drawback the human reads.</recommendation>
</open-question>
...
<open-question id="Stale tags after manual refresh" status="open">
  ...
  <alternative id="Strip and regenerate">
    The refresh run detects that a regenerated block&apos;s new recommended option differs from what surviving dependents&apos; `&lt;depends-on&gt;` elements assumed, strips those dependents&apos; embedded children transitively, and re-dispatches them within the same run.
    <advantage>Leaves the document fully self-consistent the moment the sweep exits — no block ever carries a rationale reasoned from a superseded sibling recommendation.</advantage>
    <drawback>Gives the annotate-only sweep a strip-mutation of blocks it was not asked to annotate plus an outer re-gather loop it deliberately lacks, and lets a one-block manual refresh cascade into re-recommending most of the question set.</drawback>
  </alternative>
  <alternative id="Leave for answer-time strip">
    The sweep regenerates only the hand-cleared block and leaves surviving dependents&apos; tags exactly as they are, relying on the cascade&apos;s recorded-option-vs-assumed-option comparison to strip them when the answer actually disagrees.
    <advantage>Needs no new machinery and is already correct in both branches: the tag records what that dependent&apos;s reasoning assumed, and the answer-time comparison is against the recorded answer, so a dependent is stripped exactly when the outcome contradicts it and kept when it does not — regardless of what the target&apos;s live recommendation says.</advantage>
    <drawback>Between the refresh and the answer the document holds a dependent whose declared assumption disagrees with its target&apos;s live recommendation, so answering that dependent first lifts a rationale reasoned from a superseded recommendation.</drawback>
  </alternative>
  <alternative id="Rewrite tags in place">
    The sweep edits each surviving dependent&apos;s `&lt;depends-on&gt;` `option` value to name the regenerated block&apos;s new recommended option, leaving that dependent&apos;s alternatives and rationale untouched.
    <advantage>Cheapest way to make the document read as internally consistent again — one attribute edit per dependent, no re-dispatch and no stripped work.</advantage>
    <drawback>Falsifies the record and inverts the answer-time strip: the tag then claims an assumption the untouched rationale never made, so answering with the new option reads as agreement and keeps a stale dependent, while answering with the originally assumed option strips one whose reasoning was actually sound.</drawback>
  </alternative>
  <alternative id="Advise, mutate nothing">
    The sweep compares the regenerated block&apos;s option against surviving dependents&apos; `&lt;depends-on&gt;` values and prints the mismatching Short Titles as a git-absent advisory, changing no dependent block.
    <advantage>Keeps the sweep annotate-only while telling the user exactly which dependents to hand-clear if they want them refreshed too.</advantage>
    <drawback>Adds a detection step for a failure mode no milestone has yet recorded, and cannot distinguish a stale assumption from a dependent that deliberately assumed a non-recommended option, so it will fire on legitimate mismatches.</drawback>
  </alternative>
  <recommendation option="Leave for answer-time strip">A `&lt;depends-on&gt;` records what a dependent assumed, not a pointer that must track its target&apos;s live recommendation, and since the cascade compares the assumed option against the option actually recorded, the strip is already correct without the sweep gaining any cascade machinery — and the same hand-clear escape hatch the user just used on the target covers a dependent they also want regenerated.</recommendation>
</open-question>
